...
/// <summary> 多租戶用戶實體模型 </summary>
[Table("multi_tenant_users")]
[Comment("多租戶用戶資料表")]
[Index(nameof(TenantId), nameof(Account), Name = "uq_multi_tenant_users_tenant_account", IsUnique = true)]
[Index(nameof(TenantId), nameof(Email), Name = "uq_multi_tenant_users_tenant_email", IsUnique = true)]
public class MultiTenantUserEntityModel : AuditableEntityModel
{
    ...
    public virtual ICollection<MultiTenantRoleMappingEntityModel> RoleMappings { get; set; } = [];

    /// <summary> 登入方式集合 </summary>
    public virtual ICollection<MultiTenantUserLoginEntityModel> UserLogins { get; set; } = [];
}
